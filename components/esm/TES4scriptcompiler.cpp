#include "TES4scriptcompiler.hpp"
...
namespace ESM
{
	...
	TES4ScriptCompiler::TES4ScriptCompiler(const std::string & scriptText, ESM::ESMWriter& esm, CSMDoc::Document& doc) : mScriptText(scriptText),
		mESM(esm), mDoc(doc)
	{
	...
	void TES4ScriptCompiler::processScript()
	{
		parser();
	...
	std::string TES4ScriptCompiler::GetConvertedScript()
	{
...
//		exportStr << std::endl << oldCode;
	...
	bool TES4ScriptCompiler::PerformGoodbye()
	{
		return bGoodbye;
	}

	const char* TES4ScriptCompiler::GetCompiledByteBuffer()
	{
	...
	uint32_t TES4ScriptCompiler::GetByteBufferSize()
	{
		if (HasFailed() == true)
			return 0;
		else
	...
	uint16_t TES4ScriptCompiler::LookupLocalVarIndex(const std::string & varName)
	{
		uint16_t retData = 0;
	...
	std::string TES4ScriptCompiler::lookupLocalVarType(const std::string & varName)
	{
		int retData = 0;
		retData = prepare_localvar(varName);
		if (retData == 0)
	...
	std::string TES4ScriptCompiler::lookupLocalVarType(int varIndex)
	{
	...
	void TES4ScriptCompiler::read_line(const std::string& lineBuffer)
	{
	...
	void TES4ScriptCompiler::read_quotedliteral(const std::string & lineBuffer)
	{
		bool done = false;
	...
	void TES4ScriptCompiler::read_identifier(const std::string & lineBuffer)
	{
//		bool done = false;
	...
	void TES4ScriptCompiler::read_operator(const std::string & lineBuffer)
	{
	...
	void TES4ScriptCompiler::parse_statement(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::parse_expression(std::vector<struct Token>::iterator & tokenItem)
	{
...
//			mVarRef_mID = "";
	...
	void TES4ScriptCompiler::parse_operator(std::vector<struct Token>::iterator& tokenItem)
	{
		uint8_t OpCode_Push = 0x20;
	...
	uint16_t TES4ScriptCompiler::getOpCode(std::string OpCodeString)
	{
		uint16_t OpCode=0;
	...
	void TES4ScriptCompiler::parse_placeatme(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::parse_positionCW(std::vector<struct Token>::iterator & tokenItem)
	{
		// output translated script text
		std::string cmdString = tokenItem->str;
	...
	void TES4ScriptCompiler::parse_modfactionrep(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::parse_set(std::vector<struct Token>::iterator & tokenItem)
	{
		std::string varSIG = "";
...
//		mVarRef_mID = "";
	...
	void TES4ScriptCompiler::parse_0arg(std::vector<struct Token>::iterator & tokenItem)
	{
		// output translated script text
		std::stringstream convertedStatement;
		std::string cmdString = tokenItem->str;
...
//			cmdString = "refreshtopiclist";
	...
	bool TES4ScriptCompiler::sub_parse_arg(std::vector<struct Token>::iterator &tokenItem, std::string &argString, bool &bEvalArgString, bool &bNeedsDialogHelper, std::string argSIG, bool bReturnBase)
	{
	...
	void TES4ScriptCompiler::parse_1arg(std::vector<struct Token>::iterator & tokenItem)
	{
...
//			cmdString = "HasMagicEffect";
	...
	void TES4ScriptCompiler::parse_2arg(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::parse_messagebox(std::vector<struct Token>::iterator & tokenItem)
	{
...
*/
	...
	void TES4ScriptCompiler::parse_localvar(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::parse_if(std::vector<struct Token>::iterator & tokenItem)
	{
		// start building expression
	...
	void TES4ScriptCompiler::parse_else(std::vector<struct Token>::iterator & tokenItem)
	{
		pop_context_conditional(tokenItem);
	...
	void TES4ScriptCompiler::pop_context_conditional(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	int TES4ScriptCompiler::check_get_set_mod_AV_command(const std::string & commandString)
	{
...
//			if (compareString.compare(actorvalueStrings[i]) == 0)
	...
	bool TES4ScriptCompiler::sub_parse_get_set_mod_AV_command(std::vector<struct Token>::iterator &tokenItem, int actorValue, std::string &cmdString, std::string &arg1String, std::vector<uint8_t> &arg1data)
	{
		cmdString = tokenItem->str;
	...
	void TES4ScriptCompiler::parse_endif(std::vector<struct Token>::iterator & tokenItem)
	{
		std::string endCmdString = tokenItem->str;
	...
	void TES4ScriptCompiler::parse_end(std::vector<struct Token>::iterator & tokenItem)
	{
		int offset;
	...
	void TES4ScriptCompiler::parse_begin(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::parse_keyword(std::vector<struct Token>::iterator & tokenItem)
	{
...
//			OutputDebugString(errMesg.str().c_str());
	...
	void TES4ScriptCompiler::parse_addremoveitem(std::vector<struct Token>::iterator& tokenItem, bool bRemove)
	{
		std::string cmdString = tokenItem->str;
...
//			itemEDID = ESM::ESMWriter::generateEDIDTES4(tokenItem->str, 0, "");
		...
		{
			std::stringstream errorMesg;
			errorMesg << "ERROR: TES4ScriptCompiler:parse_keyword() - unexpected token type, expected number_literalT: [" << tokenItem->type << "] token=" << tokenItem->str << std::endl;
			abort(errorMesg.str());
...
//			OpCode = 0x1052;
	...
	void TES4ScriptCompiler::parse_choice(std::vector<struct Token>::iterator& tokenItem)
	{
		...
		while (tokenItem->type != TokenType::endlineT && tokenItem != mTokenList.end())
		{
			...
			else
			{
				...
				std::stringstream errorMesg;
				errorMesg << "ERROR: TES4ScriptCompiler:parse_choice() - unexpected token type, expected string_literal: " << tokenItem->type << std::endl;
				abort(errorMesg.str());
			}
			...
			{
				std::stringstream errorMesg;
				errorMesg << "ERROR: TES4ScriptCompiler:parse_choice() - unexpected token type, expected number_literal: " << tokenItem->type << std::endl;
				abort(errorMesg.str());
			}
			tokenItem++; // advance to next token
	...
	void TES4ScriptCompiler::parse_journal(std::vector<struct Token>::iterator& tokenItem)
	{
	...
	void TES4ScriptCompiler::compile_command(uint16_t OpCode, uint16_t sizeParams)
	{
...
//			mCurrentContext.compiledCode.push_back(OpCode_Push);
	...
	void TES4ScriptCompiler::compile_command(uint16_t OpCode, uint16_t sizeParams, uint16_t countParams, std::vector<uint8_t> param1data)
	{
		compile_command(OpCode, sizeParams);
		if (sizeParams == 0)
			return;
	...
	void TES4ScriptCompiler::compile_command(uint16_t OpCode, uint16_t sizeParams, uint16_t countParams, std::vector<uint8_t> param1data, std::vector<uint8_t> param2data)
	{
	...
	std::vector<uint8_t> TES4ScriptCompiler::compile_param_long(int int_val)
	{
...
//			resultData.push_back(OpCode_Push);
	...
	std::vector<uint8_t> TES4ScriptCompiler::compile_param_float(double float_val)
	{
...
//			resultData.push_back(OpCode_Push);
	...
	std::vector<uint8_t> TES4ScriptCompiler::compile_param_varname(const std::string & varName, const std::string& sSIG, int modeflag)
	{
		uint8_t RefCode = 0;
		uint16_t RefData = 0;
		std::vector<uint8_t> resultData;
...
//				resultData.push_back(OpCode_Push);
	...
	uint16_t TES4ScriptCompiler::prepare_localvar(const std::string& varName, int mode)
	{
		uint16_t RefData = 0;
	...
	uint16_t TES4ScriptCompiler::prepare_reference(const std::string & baseName, std::string& refSIG, int mode)
	{
		uint16_t RefData = 0;
...
//				searchName = mESM.generateEDIDTES4(searchName, 0, sSIG);
	...
	std::vector<uint8_t> TES4ScriptCompiler::compile_external_localvar(const std::string &refName, const std::string &varName)
	{
		std::vector<uint8_t> varData;
		std::string searchName = Misc::StringUtils::lowerCase(varName);
		...
		TES4ScriptCompiler refScriptObj(refScriptText, mESM, mDoc);
...
//			varData.push_back(OpCode_Push);
	...
	bool TES4ScriptCompiler::lookup_reference(const std::string &baseName, std::string &refEDID, std::string &refSIG, std::string &refValString)
	{
		bool result=true;
...
//			|| refSIG_lowercase == "levc"
	...
	void TES4ScriptCompiler::gotoEOL(std::vector<struct Token>::iterator & tokenItem)
	{
	...
	void TES4ScriptCompiler::error_mesg(std::string errString)
	{
		std::stringstream errStream;
		if (mScriptName != "")
	...
	void TES4ScriptCompiler::abort(std::string error_string)
	{
		if (mFailureCode == 0)
		{
			mFailureCode = 1;
	...
	bool TES4ScriptCompiler::HasFailed()
	{
		if (mFailureCode != 0)
			return true;
	...
	void TES4ScriptCompiler::setup_dictionary()
	{
...
//		mKeywords.push_back("getalarmed");
	...
	void TES4ScriptCompiler::parser()
	{
		mParseMode = 0;
		bSetCmd = false;
	...
	void TES4ScriptCompiler::ExtractChoices()
	{
		mParseMode = 0;
		bSetCmd = false;
	...
	void TES4ScriptCompiler::ExtractLocalVars()
	{
		mParseMode = 0;
		bSetCmd = false;
	...
	bool TES4ScriptCompiler::Token::IsHigherPrecedence(const std::vector<struct Token>::iterator & tokenItem)
	{
